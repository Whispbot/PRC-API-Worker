                ...
                try
                {
                    ...
                    if (context.Request.Method == Get.ToString() && useCache)
                    {
                        ...
                        if (cachedResponse is not null)
                        {
                            ...
                            await context.Response.WriteAsJsonAsync(new { code = 304, message = "Item Cached", data = JsonSerializer.Deserialize<JsonElement>(cachedResponse), cachedAt = cachedAtMs });
                            return;
                        }
                    }
                    ...
                    if (item.complete)
                    {
                        if (item.success) // We gucci
                        {
                            ...
                            context.Response.StatusCode = StatusCodes.Status200OK;
                            await context.Response.WriteAsJsonAsync(new { code = 200, data = item.result is not null ? (JsonElement?)JsonSerializer.Deserialize<JsonElement>(item.result) : null });
                        }
                        else // Request failed for whatever reason
                        {
                            ...
                            long? cachedAtMs = cachedAt is not null ? long.Parse(cachedAt) : null;
                            context.Response.StatusCode = StatusCodes.Status400BadRequest;
                            await context.Response.WriteAsJsonAsync(new { code = item.failureCode, message = item.failureReason, data = expiredCache is not null ? (JsonElement?)JsonSerializer.Deserialize<JsonElement>(expiredCache) : null, cachedAt = cachedAtMs });
                        }
                    }
                    else // Request never completed within expected duration
                    {
                        ...
                        await context.Response.WriteAsJsonAsync(new { code = 408, message = "Request Timeout", data = expiredCache is not null ? (JsonElement?)JsonSerializer.Deserialize<JsonElement>(expiredCache) : null, cachedAt = cachedAtMs });
                    }
                } 
                catch (Exception ex)
                {
